    ...
    i18nMessage = {
        {
            LangCN,
            {
                ...
                {MsgSelectI18nJsonFolderButton,
                 ICON_FA_FILE " Select i18n json directory"},
                {
                    MsgSelectI18nJsonFolder,
...
void OperationsWindow::gui() {
  ...
  static I18NProjectStatus i18nProjectStatus =
      state->i18nJSONDir == ""
          ? (state->gameDir == "" ? I18NProjectStatus::Unknown
                                  : I18NProjectStatus::Missing)
          : I18NProjectStatus::Existed;
  static std::string i18nProjectVersion =
      state->i18nJSONDir == "" ? "" : git_head_digest(state);
  ...
  if (state->lang == LangCN) {
    ...
    if (i18nProjectStatus != I18NProjectStatus::Existed) {
      ...
      ImGui::TextColored(ImVec4(0.6875f, 0.621f, 0.539f, 1.0f),
                         _(MsgI18nProjectVersionTmpl),
    ...
    if (ImGui::Button(_(MsgSelectI18nJsonFolderButton), ImVec2(0, 40)))
      ifd::FileDialog::Instance().Open("SelectI18nJsonFolder",
                                       _(MsgSelectI18nJsonFolder), "");
  }

  if (state->i18nJSONDir.string().length() > 0) {
    ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                       _(MsgI18nJsonFolderPrefix));
    ImGuiExt::SameLine_IfPossible(10);
  ...
  // 渲染 “翻译文本” 按钮
  {
    bool enabled = state->gameDir.string().length() > 0 &&
                   state->i18nJSONDir.string().length() > 0;
    if (ImGuiExt::Button_WithEnabledFlag(_(MsgDoTranslateButton), enabled,
                                         _(MsgDoTranslateButtonTooltip),
                                         false)) {
