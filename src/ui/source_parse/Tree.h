...
template <typename T, typename Visitor_T> inline void tree_visit_breadth_first(Visitor_T visitor, Tree<T> &xs_io)
{
    visitor(xs_io);
...
template <typename T, typename Visitor_T> inline void tree_visit_depth_first(Visitor_T visitor, Tree<T> &xs_io)
{
    for (auto &child : xs_io.children_)
...
template <typename T> Tree<T> *tree_find_parent(Tree<T> &treeRoot, Tree<T> &treeNode)
{
...
inline void tree_visit_parents(Visitor_T visitor, Tree<T> &tree_root, Tree<T> &node)
{
    Tree<T> *parent = &node;
    do
    {
...
inline Tree<T> tree_sort(FunctionComparator functionComparatorT, const Tree<T> &xs)
{
    auto functionComparatorTreeT = [&functionComparatorT](const Tree<T> &v1, const Tree<T> &v2) -> bool {
        return functionComparatorT(v1.value_, v2.value_);
    };

    auto treeSortFunction = [&functionComparatorT](const Tree<T> &xxs) { return tree_sort(functionComparatorT, xxs); };

    auto sorted_children = fplus::transform(treeSortFunction, xs.children_);

    Tree<T> r{xs.value_, fplus::sort_by(functionComparatorTreeT, sorted_children)};
    return r;
}

template <typename T> inline std::string tree_show(const Tree<T> &xs)
{
...
template <typename T, typename UnaryPredicate> inline Tree<T> tree_keep_if(UnaryPredicate f, const Tree<T> &xs)
{
    Tree<T> result = xs;

    result.children_ = fplus::keep_if([f](const Tree<T> &xxs) { return f(xxs.value_); }, result.children_);
...
inline Tree<OutType> tree_transform_values(Transformer_T transformer, const Tree<T> &xs)
{
    Tree<OutType> r;
...
template <typename T> struct FlaggedValue
{
    T value;
    bool flag;
};

template <typename T> std::ostream &operator<<(std::ostream &os, const FlaggedValue<T> &v)
{
    os << "{" << v.value << " : " << v.flag << "}";
    return os;
}
...
inline Tree<T> tree_keep_wholebranch_if(UnaryPredicate f, const Tree<T> &xs)
{
    ...
    auto toFlaggedValue = [](const T &v) { return FlaggedValue<T>{v, false}; };
    Tree<FlaggedValue<T>> flaggedTree = tree_transform_values<FlaggedValue<T>>(toFlaggedValue, xs);

    // This visitor will apply the flag on a given node + all its parent and
    // descendants if the predicate returns true
    auto visitorFlagBranch = [&f, &flaggedTree](Tree<FlaggedValue<T>> &v) {
        auto visitorSetFlagOn = [&f](Tree<FlaggedValue<T>> &v) { v.value_.flag = true; };
    ...
    auto toT = [](const FlaggedValue<T> &v) { return v.value; };
    auto filteredTree = tree_transform_values<T>(toT, filteredFlaggedTree);

    return filteredTree;
}

template <typename T> T value_from_string(const std::string &s)
{
    T value;
...
template <typename T> Tree<T> tree_from_string(const std::string &s, int indentation = 2)
{
    Tree<T> root;
