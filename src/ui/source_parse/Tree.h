...
template <typename T, typename Visitor_T>
inline void tree_visit_parents(Visitor_T visitor, Tree<T> &tree_root,
                               Tree<T> &node) {
  Tree<T> *parent = &node;
  do {
...
inline Tree<T> tree_sort(FunctionComparator functionComparatorT,
                         const Tree<T> &xs) {
  auto functionComparatorTreeT =
      [&functionComparatorT](const Tree<T> &v1, const Tree<T> &v2) -> bool {
  ...
  Tree<T> r{xs.value_,
            fplus::sort_by(functionComparatorTreeT, sorted_children)};
  return r;
}
...
template <typename T> inline std::string tree_show(const Tree<T> &xs) {
  ...
  auto indentLines = [](const std::string &s, int nbSpaces) -> std::string {
    ...
    auto indentedLines = fplus::transform(
        [indentation](const auto &ss) { return indentation + ss; }, lines);
  ...
  if (!xs.children_.empty()) {
    ...
    r = r + childStart +
        indentLines(fplus::join(siblingSeparator, strings), 2) + childEnd;
  }
  return r;
}
...
inline Tree<T> tree_keep_if(UnaryPredicate f, const Tree<T> &xs) {
  ...
  result.children_ = fplus::keep_if(
      [f](const Tree<T> &xxs) { return f(xxs.value_); }, result.children_);
...
inline Tree<OutType> tree_transform_values(Transformer_T transformer,
                                           const Tree<T> &xs) {
  Tree<OutType> r;
...
inline Tree<T> tree_keep_wholebranch_if(UnaryPredicate f, const Tree<T> &xs) {
  ...
  Tree<FlaggedValue<T>> flaggedTree =
      tree_transform_values<FlaggedValue<T>>(toFlaggedValue, xs);
  ...
  auto filteredFlaggedTree =
      tree_keep_if([](auto v) { return v.flag; }, flaggedTree);
...
Tree<T> tree_from_string(const std::string &s, int indentation = 2) {
  ...
  for (const auto &line : lines) {
    int line_depth =
        fplus::take_while([](auto c) { return c == ' '; }, line).size() /
        indentation;
