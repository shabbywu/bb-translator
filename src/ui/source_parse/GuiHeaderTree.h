...
protected:
  ...
  bool mShowToc = true;
  bool mScrollToSelectedNextTime =
      false; // only valid for "follow" mode (in subclass)

  enum class ExpandCollapseAction { ExpandAll, CollapseAll, NoAction };
...
public:
  GuiHeaderTree_FollowDemo(const LinesWithTags &linesWithTags)
  ...
  int gui(int currentEditorLineNumber) override {
    ImGui::SameLine();
    return GuiHeaderTree::gui(GImGuiDemoMarker_IsActive
                                  ? mCurrentFollowedLine
