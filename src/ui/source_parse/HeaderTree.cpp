...
HeaderTree *searchParent(HeaderTree *treeRoot, HeaderTree *currentNode,
...
void makeHeaderTrees_Impl(std::deque<LineWithTag> &headerLinesStream,
                          HeaderTree *treeRoot,
  ...
  // Search for correct parent
  {
    if (nextHeader.level > outHeaderTreeParent->value_.level) {
      ...
      parent =
          tree_find_parent(*treeRoot, *outHeaderTreeParent); // Arg, bad perf
    } else // if (nextHeader.level < outHeaderTreeParent.value_.level)
    {
