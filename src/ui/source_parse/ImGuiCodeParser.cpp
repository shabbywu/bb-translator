...
auto isHeader = [](const NumberedLines &numberedLines, size_t idxLine) -> HeaderLineInfo {
...
LinesWithTags findImGuiHeaderDoc(const std::string &sourceCode)
{
    ...
    // mess")
    NumberedLines indexableLines =
        fplus::drop_while([](const auto &line) { return !isTopHeaderDashes(line); }, numberedLines);

    auto cleanTitle = [](const HeaderLineInfo &headerLineInfo) {
        // cut title after first sentence ends (i.e after '.', '?', '!')
        std::string title =
            fplus::take_while([](auto c) { return c != '.' && c != '!' && c != '?'; }, headerLineInfo.title);
    ...
    for (size_t i = 0; i < indexableLines.size(); ++i)
    {
        ...
        if (headerLineInfo.headerLevel > 0)
        {
            ...
            linesWithTags.push_back({lineNumber, cleanTitle(headerLineInfo), headerLineInfo.headerLevel});
        }
    }
