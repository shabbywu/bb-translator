...
bool isInnerCommentLine(const NumberedLine &line) {
  ...
  if (!fplus::is_prefix_of("  "s, lineCode))
    return false;
  return fplus::is_prefix_of(std::string("//"),
                             fplus::trim_whitespace_left(lineCode));
}
...
bool isInnerHeaderDashes(const NumberedLine &line) {
  ...
  if (!fplus::is_prefix_of("  "s, line.second))
    return false;
  return fplus::is_prefix_of(headerMark,
                             fplus::trim_whitespace_left(line.second));
}
...
structure.
// Access via ImGui::GetIO(). Read 'Programmer guide' section in .cpp file for
general usage.
//-----------------------------------------------------------------------------
 */
HeaderLineInfo isH1Header(const NumberedLines &numberedLines, size_t idxLine) {
  ...
  auto followingCommentLines =
      fplus::take_while(isTopCommentOrHeaderDashes, remainingLines);
...
ImGuiInputTextFlags_Callback* flag is used.
...
also follow symbols in comments. typedef int ImGuiCol;               // -> enum
ImGuiCol_             // Enum: A color identifier for
*/
HeaderLineInfo isH2Header(const NumberedLines &numberedLines, size_t idxLine) {
  ...
  auto remainingLines = fplus::drop(idxLine, numberedLines);
  auto followingCommentLines =
      fplus::take_while(isTopCommentLine, remainingLines);
  ...
  if (nextSourceLineIdx < numberedLines.size() - 1) {
    ...
    if (fplus::is_prefix_of("struct "s, nextSourceLine) &&
        fplus::is_prefix_of("{"s, nextSourceLine2))
...
// H3 headers look like this (note the spacing at the beginning of lines; since
// we are in a namespace or in a struct.
/*
    //------------------------------------------------------------------
...
HeaderLineInfo isH3Header(const NumberedLines &numberedLines, size_t idxLine) {
  ...
  auto remainingLines = fplus::drop(idxLine, numberedLines);
  auto followingCommentLines =
      fplus::take_while(isInnerCommentLine, remainingLines);
  auto lastCommentLine = followingCommentLines.back();
...
HeaderLineInfo isH4Header(const NumberedLines &numberedLines, size_t idxLine) {
  ...
  // return NotAHeader();
  std::string trimmedPreviousLine =
      fplus::trim_whitespace(numberedLines[idxLine - 1].second);
...
auto isHeader = [](const NumberedLines &numberedLines,
                   size_t idxLine) -> HeaderLineInfo {
...
LinesWithTags findImGuiHeaderDoc(const std::string &sourceCode) {
  ...
  // imgui.h file content becomes indexable after the first H1 title ("Header
  // mess")
  NumberedLines indexableLines = fplus::drop_while(
      [](const auto &line) { return !isTopHeaderDashes(line); }, numberedLines);
  ...
  for (size_t i = 0; i < indexableLines.size(); ++i) {
    ...
    if (headerLineInfo.headerLevel > 0) {
      ...
      linesWithTags.push_back(
          {lineNumber, cleanTitle(headerLineInfo), headerLineInfo.headerLevel});
    }
  }
...
std::string lowerCaseExceptFirstLetter(const std::string &s) {
  auto r = fplus::to_upper_case(fplus::take(1, s)) +
           fplus::to_lower_case(fplus::drop(1, s));
  return r;
}
