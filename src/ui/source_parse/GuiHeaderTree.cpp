...
int GuiHeaderTree::guiImpl(int currentEditorLineNumber, const HeaderTree &headerTree, bool isRootNode)
{
    ...
    std::string title = lineWithTag.tag + "##" + std::to_string(lineWithTag.lineNumber);
...
bool DrawImGuiTextFilterWithTooltip(ImGuiTextFilter &imGuiTextFilter,
                                    const std::string &tooltipText =
                                        "Filter usage:[-excl],incl\n"
                                        "For example:\n"
                                        "   \"button\" will search for \"button\"\n"
                                        "   \"-widget,button\" will search for \"button\" without \"widget\"",
                                    const std::string &filterLabel = "Filter usage:[-excl],incl")
{
...
{
    auto lambdaPassFilter = [this](const LineWithTag &t) { return mFilter.PassFilter(t.tag.c_str()); };
    mFilteredHeaderTree = tree_keep_wholebranch_if(lambdaPassFilter, mHeaderTree);
}
