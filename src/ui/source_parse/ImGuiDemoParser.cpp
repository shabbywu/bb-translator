    ...
    */
    {
        auto endLineNumber = fplus::find_first_idx_by(
            [](const NumberedLine &line) { return line.second == "// End of Demo code"; }, numberedLines);
    ...
    auto extractSectionName = [&numberedLines](const std::pair<size_t, size_t> &sectionScope) -> std::string {
    ...
    for (const auto &sectionScope : sectionScopes)
    {
        ...
        {
            auto sectionNumberedLines = fplus::get_segment(sectionScope.first, sectionScope.second, numberedLines);
