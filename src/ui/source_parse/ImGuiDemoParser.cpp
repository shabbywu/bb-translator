...
LinesWithTags findImGuiDemoCodeLines(const std::string &sourceCode) {
  ...
  auto demoCodeLines = fplus::keep_if(
      [](const NumberedLine &v) {
        return fplus::is_prefix_of("IMGUI_DEMO_MARKER("s,
                                   fplus::trim_whitespace_left(v.second));
      },
      numberedLines);

  LinesWithTags tags = fplus::transform(
      [](const NumberedLine &v) {
        LineWithTag r;
        // codeLine look like "    IMGUI_DEMO_MARKER("Menu/Tools");"
        // And for a tag like this, the title should be "Tools", and the level
        // should be 2
  ...
  for (auto tag_copy : tags) {
    std::vector<std::string> tag_levels =
        fplus::split('/', false, tag_copy.tag);
...
//-----------------------------------------------------------------------------
  ...
  auto numberedLines =
      fplus::enumerate(fplus::split_lines(true, sourceFile.sourceCode));
  auto isLineExampleAppSectionStarts =
      [&numberedLines](const NumberedLine &numberedLine) {
  ...
  auto idxLineStartExampleSections =
      fplus::find_all_idxs_by(isLineExampleAppSectionStarts, numberedLines);

  // Step 2: add an additional index which is the end of the demo code in
  // imgui_demo.cpp it is denoted by the following line:
  /*
// End of Demo code
  */
  {
    auto endLineNumber = fplus::find_first_idx_by(
  ...
  std::vector<std::pair<size_t, size_t>> sectionScopes =
      fplus::overlapping_pairs(idxLineStartExampleSections);

  // Step 4: Gather the sections into a map
  std::unordered_map<std::string, SourceCode> exampleAppsCodes;
  auto extractSectionName =
      [&numberedLines](
          const std::pair<size_t, size_t> &sectionScope) -> std::string {
  ...
  for (const auto &sectionScope : sectionScopes) {
    ...
    {
      auto sectionNumberedLines = fplus::get_segment(
          sectionScope.first, sectionScope.second, numberedLines);
      std::vector<std::string> sectionLines = fplus::transform(
          [](const auto &v) { return v.second; }, sectionNumberedLines);
