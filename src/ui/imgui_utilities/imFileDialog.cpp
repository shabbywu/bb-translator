...
bool FileDialog::Open(const std::string &key, const std::string &title, const std::string &filter, bool isMultiselect,
                      const std::string &startingDir)
{
    if (!m_currentKey.empty())
        return false;
    ...
    for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++)
    {
        ...
        if (data.Path.has_extension())
        {
            ...
            if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga")
            {
                ...
                unsigned char *image =
                    stbi_load(data.Path.string().c_str(), &width, &height, &nrChannels, STBI_rgb_alpha);
...
void FileDialog::m_renderContent()
{
    ...
    if (m_zoom == 1.0f)
    {
                              ...
                              /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN)))
        {
            ...
            ImGui::TableSetupColumn("Size##filesize", ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize,
                                    0.0f, 2);
            ...
            for (auto &entry : m_content)
            {
                ...
                if (tm != nullptr)
                    ImGui::Text("%d/%d/%d %02d:%02d", tm->tm_mon + 1, tm->tm_mday, 1900 + tm->tm_year, tm->tm_hour,
                                tm->tm_min);
                else
                    ImGui::Text("---");
...
void FileDialog::m_renderFileDialog()
{
    ...
    if (ImGui::InputTextEx("##searchTB", "Search", m_searchBuffer, 128, ImVec2(-FLT_MIN, GUI_ELEMENT_SIZE),
