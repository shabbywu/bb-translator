...
int gui_main() {
  {
    ...
    if (std::filesystem::exists(current_path / "data") &&
        std::filesystem::exists(current_path / "win32")) {
    ...
    if (ImGui::BeginMenu(_(MsgMenuLinkTitle))) {
      ...
      if (ImGui::MenuItem(_(MsgMenuLinkCNProject)))
        HyperlinkHelper::OpenUrl(
            "https://github.com/shabbywu/Battle-Brothers-CN");
  ...
  runnerParams.callbacks.ShowStatus = [] {
    MarkdownHelper::Markdown(_(MsgStatusBar));
  };
  ...
  runnerParams.callbacks.BeforeImGuiRender = [] {
    ...
    HelloImGui::ImageFromAsset("background.jpeg",
                               ImVec2(size[0] + 20, size[1] + 20));
    ImGui::End();
  ...
  runnerParams.callbacks.PostInit_AddPlatformBackendCallbacks = []() {
    std::filesystem::path userConfigFolder = HelloImGui::IniFolderLocation(
        HelloImGui::IniFolderType::AppUserConfigFolder);
...

    {
      ...
      if (!std::filesystem::exists(backgroundFilename)) {
        ...
        std::ofstream f(backgroundFilename,
                        std::ios::out | std::ios::binary | std::ios::trunc);
        if (f.fail())
          return;
...
int main(int argc, char *argv[]) {
  ...
  program.add_argument("--game-dir")
      .help("specify the game root directory")
      .default_value("");

  program.add_argument("--i18n-json-dir")
      .help("specify the i18n json directory")
      .default_value("");
  ...
  std::filesystem::path i18nJsonDir =
      program.get<std::string>("--i18n-json-dir");
