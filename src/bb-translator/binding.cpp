...
void setup_python(AppState *state) {
  {
    ...
    if (!std::filesystem::exists(state->pythonRootDir /
                                 pybaseLibrary.getFileName())) {
      auto pybaseLibraryPath =
          state->pythonRootDir / pybaseLibrary.getFileName();
      std::ofstream f(pybaseLibraryPath,
    ...
    if (!std::filesystem::exists(state->pythonRootDir / ".binary.unzip.v1")) {
      ...
      zip_stream_extract(pythonZip.getBuffer(), pythonZip.getSize(), c_path,
                         nullptr, nullptr);
    ...
    auto pythonPath =
        pythonRootDir + L"\\base_library.zip;" + pythonRootDir + L";";
#else
    auto pythonHome = pythonRootDir + L"/";
    auto pythonPath = pythonRootDir + L"/base_library.zip:" + pythonRootDir +
                      L":" + pythonRootDir + L"/lib-dynload:";
#endif
...
void dispatch_translator(AppState *state) {
  ...
  try {
    if (mtx.try_lock()) {
      ...
      py::dict locals;
      locals["task"] =
          py::make_tuple(state->gameDir.string(), state->i18nJSONDir.string());
      py::exec(R"(
                if queues:
                    addLog("上一个任务尚未完成...")
                else:
